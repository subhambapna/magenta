...
This option (false by default) turns on the out-of-memory (OOM) kernel
thread, which kills processes when the PMM has less than `oom.redline_mb` free
memory, sleeping for `oom.sleep_sec` between checks.
